...
> 用于存储访问令牌信息，当成功获取到令牌信息时，会生成一条记录
...
### 2.4 oauth_approvals(用户授权表)

> 主要用于存储用户的授权信息，比如授权范围、授权状态、授权期限等
...
### 2.5 oauth_client_details(客户端详情表)

> 主要用于存储客户端详细信息，比如：权限信息、访问密钥、授权类型、权限范围等
